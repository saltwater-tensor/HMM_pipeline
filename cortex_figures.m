          ...
          for atname = 1:1:length(surface_file_data.Atlas)
             if strcmpi(surface_file_data.Atlas(atname).Name,atlas_name)
                 ...
                 for sc = 1:1:length(ROIs) %length(Scouts)
                    ...
                    custom_Scouts(ROIs(sc)).Vertices = Scouts(ROIs(sc)).Vertices;
                    custom_Scouts(ROIs(sc)).spatialPC = V(:,end);
                    custom_Scouts(ROIs(sc)).var_explained = Lm(end);
                    custom_Scouts(ROIs(sc)).Label = Scouts(ROIs(sc)).Label;
...
%% 
 Amp_file = zeros(15002,2);
    ...
    % Output filename
    OPTIONS.ResultFile = fullfile([database 'data\Group_analysis\'], ['results_Group_trial'] );
    % Save file
