function h = schemaball(r, lbls, ccolor, ncolor, szdata,colr_mat_2_orig,figstring)
...
% Create figure
figure('renderer','painter','visible','on')
axes('NextPlot','add')
...
s.s        = scatter(x(isrt),y(isrt),[], ncolor(isrt,:),'fill','MarkerEdgeColor',ecolor,'LineWidth',1);
s.s.SizeData = 10*szdata(isrt);
...
ipos       = x > 0;
s.t        = zeros(sz(1),1);
s.t( ipos) = text(x( ipos)*1.1, y( ipos)*1.1, lbls( ipos),'Color',tcolor,'FontWeight','bold','FontSize',18);
...
set(gcf,'Name',figstring)
axis equal

% Set colormap
colormap(gca,ccolor);
hold on
scatter(x(isrt),y(isrt),100, colr_mat_2_orig(isrt,:),'fill','MarkerEdgeColor','flat','LineWidth',1);
if nargout == 1
    h = s;
end
